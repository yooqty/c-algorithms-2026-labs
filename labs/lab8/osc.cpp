...
{   
    ...
    if (P_P)   
        delete P_P;   
    P_P = new Ca_Line(2, plot2, 0, 0, FL_BLUE, CA_NO_POINT);   
    ...
    {   
        ...
        {   
            ...
            for (i = 0;  i < 512;  i++)  
            {   
                plot[2*i] = plots[2*i]= i;  
                plot [2*i + 1] = capture [j  ];   
                plots[2*i + 1] = capture2[j++];   
                if (j >= CAPTURE_SIZE)   
                    j = 0;   
            }   
            last_used_sample = j;  
            ...
            if (S_S)   
                delete S_S;   
            S_S = new Ca_Line(512, plots, 0, 0, FL_RED, CA_NO_POINT);   
        }   
    }   
    else   
    {   
...
{
	scanf("%lf%lf", &Y, &Y1);
	inbuf[j]=(uint8_t)  ((Y /2+70)+50);
	inbuf2[j]=(uint8_t) ((Y1/2+70)+50);
}
  len=1024;
    	    if (len > 0)   
        {   
            ...
            {   
                ...
                if (skip >= x_scaling)   
                {   
                    capture2[next_capture_sample] = inbuf2[i]*16.0;   
